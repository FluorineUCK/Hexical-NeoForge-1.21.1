...
object OpGrokPull : ConstMediaAction {
	...
	override fun execute(args: List<Iota>, env: CastingEnvironment): List<Iota> {
		...
		val stack = image.stack.map { if (MishapOthersName.getTrueNameMishapFromDatum(env.world, it, player) == null) it else NullIota() }
		val parentheized = image.parenthesized.map { if (MishapOthersName.getTrueNameMishapFromDatum(env.world, it.iota, player) == null) it.iota else NullIota() }
