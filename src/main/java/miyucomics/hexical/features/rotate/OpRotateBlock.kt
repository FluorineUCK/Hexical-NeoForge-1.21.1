...
object OpRotateBlock : SpellAction {
	...
	override fun execute(args: List<Iota>, env: CastingEnvironment): SpellAction.Result {
		...
		when (val result = BlockRotationHandlerRegistry.modify(state, direction)) {
			...
			else -> return SpellAction.Result(Spell(pos, result), MediaConstants.DUST_UNIT / 8, listOf(ParticleSpray.burst(pos.getCenter(), 1.0)))
		}
	}
	...
	private data class Spell(val pos: BlockPos, val state: BlockState) : RenderedSpell {
		override fun cast(env: CastingEnvironment) {
			...
			env.world.blockUpdated(pos, state.block)
		}
	}
}