...
object HexicalActions {
	private var activeRegistrar: ((ResourceLocation, ActionRegistryEntry) -> Unit)? = null

	fun registerAll(registrar: (ResourceLocation, ActionRegistryEntry) -> Unit) {
		check(activeRegistrar == null) { "Hexical actions are already being registered" }
	...
	private fun registerEntries() {
		...
		register("get_hand_lamp_use_time", "qwddedqwddwa", HexDir.SOUTH_WEST, OpGetHandLampData { env, nbt -> (env.world.gameTime - (nbt.getDouble("start_time") + 1.0)).asActionResult })
		register("get_hand_lamp_media", "qwddedaeeeee", HexDir.SOUTH_WEST, OpGetHandLampData { env, _ -> ((env.castingEntity!!.useItem.item as HandLampItem).getMedia(env.castingEntity!!.useItem).toDouble() / MediaConstants.DUST_UNIT).asActionResult })
		register("get_hand_lamp_storage", "qwddedqwaqqqqq", HexDir.SOUTH_WEST, OpGetHandLampData { env, nbt -> listOf(miyucomics.hexical.hexcompat.deserializeIotaOrThrow(nbt.getCompound("storage"), env.world)) })
		...
		register("get_arch_lamp_use_time", "qaqwddedqwddwa", HexDir.NORTH_EAST, OpGetArchLampData { ctx, data -> (ctx.world.gameTime - (data.time + 1)).asActionResult })
		register("get_arch_lamp_storage", "qaqwddedqwaqqqqq", HexDir.NORTH_EAST, OpGetArchLampData { ctx, data -> listOf(miyucomics.hexical.hexcompat.deserializeIotaOrThrow(data.storage, ctx.world)) })
		...
		register("pling", "eqqqada", HexDir.NORTH_EAST, OpHallucinateSound(BuiltInRegistries.SOUND_EVENT.wrapAsHolder(SoundEvents.PLAYER_LEVELUP)))
		register("click", "eqqadaq", HexDir.NORTH_EAST, OpHallucinateSound(BuiltInRegistries.SOUND_EVENT.wrapAsHolder(SoundEvents.UI_BUTTON_CLICK.value())))
		...
		register("ghast_fireball", "wqqqqqwaeaeaeaeae", HexDir.SOUTH_EAST, OpConjureEntity(MediaConstants.DUST_UNIT * 3) { world, position, caster ->
			val fireball = if (caster == null)
				LargeFireball(EntityType.FIREBALL, world)
			else
				LargeFireball(world, caster, net.minecraft.world.phys.Vec3.ZERO, 1)
	...
	private fun register(name: String, signature: String, startDir: HexDir, action: Action) {
		...
		checkNotNull(activeRegistrar) { "HexicalActions.registerAll must own registration" }(HexicalMain.id(name), entry)
	}
}
