...
import miyucomics.hexical.features.rotate.BlockRotationHandlerRegistry
...
object HexicalHooksServer {
	...
	fun createRegistries(event: NewRegistryEvent) {
		MageBlockModifierRegistry.createRegistry(event)
		PrestidigitationHandlersHook.createRegistry(event)
	}

	fun registerContent(event: RegisterEvent) {
		MageBlockModifierRegistry.register(event)
		PrestidigitationHandlersHook.register(event)
		TransmutingHelper.register(event)
		ScarabHandler.register(event)
		WooleyedEffectRegister.register(event)
		SentinelBedPoi.register(event)
	}
}
